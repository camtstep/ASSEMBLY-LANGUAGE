...
#            intArr[valsToDo - i] = GetOneIntByVal(entIntPrompt);		
	...
	####################(8)#################### COMPLETED
	addi $a0, $sp, 30	# move entIntPrompt to $a0
	jal GetOneIntByVal	# call GetOneIntByVal, input is in $v0
	lw $t0, 45($sp)		# $t0 has valsToDo
	sub $t0, $t0, $t1	# $t0 has valsToDo - i
	sll $t0, $t0, 2		# $t0 has (valsToDo - i)*4
	addi $t0, $t0, 49	# $t0 has ((valsToDo - i)*4) + 49(where valsToDo is located)
	add $t0, $t0, $sp	# $t0 has intArr[valsToDo - i]
	sw $v0, 0($t0)		# move $v0 to intArr[valsToDo - i]
...
# Register Usage:		
#################		
# $a2: holder of content to help swap
# $a3: holder of content to help swap
#################		
#   int temp = *intPtr1;		
#  *intPtr1 = *intPtr2;		
#   *intPtr2 = temp;		
